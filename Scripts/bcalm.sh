...
pc_working_dir="${working_dir}/bcalm"
...
adb push ${tools_dir}/bcalm.ae /data/local/tmp/bcalm_e

echo "[Setup] Get and push monitor arm"
adb push ${tools_dir}/monitor.sh /data/local/tmp/monitor.sh

## Check bcalm and monitor.sh
pred='adb shell "[ -e "/data/local/tmp/bcalm_e" ] && echo "$?""'
check_predicate "$pred" "bcalm arm install ok" "bcalm arm error while installing"

pred='adb shell "[ -e "/data/local/tmp/monitor.sh" ] && echo "$?""'
check_predicate "$pred" "monitor.sh arm install ok" "monitor.sh not found"

echo "[Setup] chmod +x monitor.sh bcalm_e"
adb shell "chmod +x /data/local/tmp/bcalm_e /data/local/tmp/monitor.sh"
...
do
	...
	echo "[${i}] BCALM running"
	set -x # printing the command
	adb shell "{ time ( cd $sp_working_dir && /data/local/tmp/bcalm_e -in ${i}.fastq -kmer-size 31 -abundance-min 2 -out-dir $sp_working_dir -out-tmp $sp_working_dir -minimizer-type 1 -max-memory 800 -storage-type file > $sp_working_dir/${i}.log 2>/dev/null ); } 2> $sp_working_dir/${i}_time.log & /data/local/tmp/monitor.sh bcalm_e > $sp_working_dir/${i}_ram.log"
	set +x
